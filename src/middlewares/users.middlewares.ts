    ...
    options: async (value: string, { req }) => {
      try {
        ...
        const decoded_forgot_password_token = await verifyToken({
          token: value,
          secretOnPublicKey: envConfig.jwtSecretForgotPasswordToken as string
        })
          ...
          options: async (value: string, { req }) => {
            try {
              ...
              const [decoded_refresh_token, refresh_token] = await Promise.all([
                verifyToken({ token: value, secretOnPublicKey: envConfig.jwtSecretRefreshToken as string }),
          ...
          options: async (value: string, { req }) => {
            ...
            try {
              const decoded_email_verify_token = await verifyToken({
                token: value,
                secretOnPublicKey: envConfig.jwtSecretEmailVerifyToken as string
              })
      ...
      username: {
        ...
        custom: {
          options: async (value: string, { req }) => {
            ...
            //Nếu tồn tại username này trong db
            // Thì cta ko cho phép update
            if (user) {
