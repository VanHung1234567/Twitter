...
class UsersService {
  ...
  private decodeRefreshToken(refresh_token: string) {
    return verifyToken({
      token: refresh_token,
      secretOnPublicKey: envConfig.jwtSecretRefreshToken as string
    })
  }
  ...
  async oauth(code: string) {
    ...
    const usertInfo = await this.getGoogleUserInfo(access_token, id_token)
    if (!usertInfo.verified_email) {
      throw new ErrorWithStatus({
        message: USERS_MESSAGES.GMAIL_NOT_VERIFIED,
        status: HTTP_STATUS.BAD_REQUEST
      })
    }
    // Kiếm tra email đã đc đăng ký hay chưa
    const user = await databaseService.users.findOne({ email: usertInfo.email })
    if (user) {
      const [access_token, refresh_token] = await this.signAccessAndRefreshToken({
